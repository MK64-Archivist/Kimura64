...
void SetGraphCPUTime(enum ProfilerGameEvent eventID) {
...
#else
GLOBAL_ASM("asm/non_matchings/profiler/SetGraphicRCPTime.s")
#endif

void SetGraphicRCPTime(void) {
...
void draw_profiler_bar(OSTime clockBase, OSTime clockStart, OSTime clockEnd, s16 posY, u16 color) {
    ...
    if (rectX1 < rectX2) {
        ...
        gDPSetFillColor(gGraphPtr++, color << 16 | color);
        gDPFillRectangle(gGraphPtr++, rectX1, posY, rectX2, posY + 2);
    }
}
...
extern struct Controller *gCont1P;

// Similar to draw_screen_borders from SM64, with a hint of draw_profiler
void resource_display(void) {
    gDPPipeSync(gGraphPtr++);
    gDPSetScissor(gGraphPtr++, G_SC_NON_INTERLACE, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
    gDPSetRenderMode(gGraphPtr++, G_RM_OPA_SURF, G_RM_OPA_SURF2);
    gDPSetCycleType(gGraphPtr++, G_CYC_FILL);
    gDPSetFillColor(gGraphPtr++, GPACK_RGBA5551(0, 0, 0, 0) << 16 | GPACK_RGBA5551(0, 0, 0, 0));

    if ((gCont1P->buttonPressed & 0x20) != 0) {
        D_800DC664 ^= 1;
    }
